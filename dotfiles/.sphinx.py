...
import sphinx
from abjad import TimespanList
...
class LilyPondExtension(Extension):
    ...
    def to_docutils(self):
        ...
        if isinstance(self.illustrable, TimespanList):
            if self.illustrable[0].annotation is not None:
        ...
        stylesheet = self.stylesheet or "abjad.ily"
    ...
    if node.get("pages"):
        for page_spec in node["pages"].split(","):
            ...
            for page_number in range(start_page, stop_page):
                for path in output_directory.glob(f"{render_prefix}-{page_number}.png"):
                    paths_to_embed.append(path)
    elif node.get("no-trim"):
        for path in output_directory.glob(f"{render_prefix}.png"):
            paths_to_embed.append(path)
        if not paths_to_embed:
            page_count = len(list(output_directory.glob(f"{render_prefix}-*.png")))
            for page_number in range(1, page_count + 1):
                for path in output_directory.glob(f"{render_prefix}-{page_number}.png"):
                    paths_to_embed.append(path)
    else:
        for path in output_directory.glob(f"{render_prefix}.cropped.png"):
            paths_to_embed.append(path)
