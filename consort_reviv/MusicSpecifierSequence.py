...
class MusicSpecifierSequence(AbjadValueObject):
    ...
    def __call__(
        ...
        timespan_specifier = timespan_specifier or \
            evans.consort_reviv.TimespanSpecifier.TimespanSpecifier()
        seed = seed or 0
        ...
        iterator = evans.consort_reviv.iterate_nwise.iterate_nwise(offsets)
        for i, offset_pair in enumerate(iterator):
            start_offset, stop_offset = offset_pair
            music_specifier = self[seed % len(self)]
            timespan = evans.consort_reviv.PerformedTimespan.PerformedTimespan(
            ...
            if not division_masks:
                ...
                if output_mask is None:
                    timespans.append(timespan)
                elif isinstance(output_mask, abjadext.rmakers.SustainMask):
                    timespans.append(timespan)
                elif isinstance(output_mask, abjadext.rmakers.SilenceMask):
                    pass
        ...
        if padding:
            ...
            for shard in timespans.partition(True):
                silent_timespan_one = evans.consort_reviv.SilentTimespan.SilentTimespan(
                    layer=layer,
                ...
                silent_timespan_two = evans.consort_reviv.SilentTimespan.SilentTimespan(
                    layer=layer,
