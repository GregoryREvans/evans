for voice in abjad.select(score).components(abjad.Voice):
    ...
    for leaf in abjad.select(voice).leaves():
        if leaf.written_duration <= abjad.Duration(7, 32):
            ...
            if all(dur >= abjad.Duration(1, 4) for dur in (prev_dur, next_dur)) or all(
                isinstance(l, abjad.Rest) for l in (previous_leaf, next_leaf)
            ):
                continue
            if prev_dur >= abjad.Duration(1, 4) or isinstance(
                previous_leaf, abjad.Rest
            ):
