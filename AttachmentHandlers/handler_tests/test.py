...
time_signatures = [abjad.TimeSignature(pair) for pair in [(4, 4)]]
...
rmaker = abjadext.rmakers.TaleaRhythmMaker(
    talea=abjadext.rmakers.Talea(counts=[2, 1, 5, 1, 3, 3, 1, 2], denominator=16),
...
articulation_handler = ArticulationHandler(
    articulation_list=["tenuto", "staccato", "portato", "tremolo"], continuous=True
)
...
dynamic_handler = DynamicHandler(
    starting_dynamic="f", hairpin=">", ending_dynamic="p", continuous=True
)
...
pitch_handler = PitchHandler(
    pitch_list=[0, 2, 1, [3, 10], 4, 8, [7, 9], 6], continuous=True
)

slur_handler = SlurHandler(slurs="runs")
...
instruments = cyc([abjad.Violin(), abjad.Violin(), abjad.Viola(), abjad.Cello()])

clefs = cyc(
    [abjad.Clef("treble"), abjad.Clef("treble"), abjad.Clef("varC"), abjad.Clef("bass")]
)
