...
for tuplet in abjad.select(staff).components(abjad.Tuplet):
    ...
    literal = abjad.LilyPondLiteral(r'\tweak TupletNumber.text #(tuplet-number::append-note-wrapper' + f'(tuplet-number::non-default-tuplet-fraction-text {imp_den} {imp_num}) "{notehead_wrapper}")', format_slot='before')
