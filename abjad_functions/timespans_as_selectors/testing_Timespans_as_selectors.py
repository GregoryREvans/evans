...
target_timespan = abjad.Timespan(start_offset=(0, 8), stop_offset=(1, 2))
...
for group in abjad.select(staff).logical_ties(pitched=True).group_by(predicate = lambda x: abjad.inspect(x).timespan().happens_during_timespan(target_timespan)):
    if abjad.inspect(group).timespan().happens_during_timespan(target_timespan) is True: ##THIE WORKS TOO, BUT IS IT FASTER?
        for tie in abjad.select(group).logical_ties(pitched=True):
            for leaf in abjad.select(tie).leaves():
                leaf.written_pitch = leaf.written_pitch + 2
