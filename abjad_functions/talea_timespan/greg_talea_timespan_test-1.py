...
rmaker_two = rmakers.TaleaRhythmMaker(
    talea=rmakers.Talea(
        counts=[1, 1, 3, -1, 2, 5, 4, 1, 3, 1, 2, 2, ],
        denominator=16,
        ),
    beam_specifier=rmakers.BeamSpecifier(
        beam_divisions_together=True,
...
articulation_handler_one = ArticulationHandler(articulation_list=['accent', ], continuous=True)
...
articulation_handler_two = ArticulationHandler(articulation_list=['tenuto', ], continuous=True)
...
rmakers_cycle = timespan_functions.cyc([rmodel_one, rmodel_one, rmodel_two, rmodel_two, rmodel_three, rmodel_three, ])

talea = rmakers.Talea(
    counts=[3, 5, -1, 6, -5, 4, -2, 8, 2, 3, -3, 5, 3, -7, 2],
    denominator=8,
)
    ...
    abjad.StaffGroup(
        [
            abjad.Staff([abjad.Voice(name=voice_name)],
                        name=staff_name, lilypond_type='Staff')
