    ...
    for timespan in timespan_list:
        if isinstance(timespan, abjad.AnnotatedTimespan):
            timespan.annotation = timespan_functions.TimespanSpecifier(
                voice_name="Voice " + str(i), rhythm_maker=next(rmakers_cycle)
            )
        else:
            timespan.annotation = timespan_functions.TimespanSpecifier(
                voice_name="Voice " + str(i), rhythm_maker=silence_maker
            )
    timespan_list.sort()
...
all_timespans = {
    voice: timespan_list for voice, timespan_list in zip(voices, dummy_list)
}
time_signatures = [
    abjad.TimeSignature(pair)
    for pair in [timespan.duration.pair for timespan in all_timespans["Voice 1"]]
]
...
# Score
voice_staff_names = [
    ["Voice " + str(i + 1), "Staff " + str(i + 1)] for i in range(master_length)
]
score = abjad.Score(
    [
